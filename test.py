...
def create_balanced_teams(pop, config, elites=None, n_teams=10):
    ...
    neuron_scores = getattr(create_balanced_teams, 'neuron_scores', {})
...
def run_qd_with_tweaks(config):
    """Run modified QD algorithm with improvements for convergence"""
    # Set random seeds
    ...
    archives = {
        neuron.neuron_id: GridArchive(
            ...
            ranges=[(0, 1), (0.8, 1)],  # Normalized descriptor range
            seed=config["seed"] + neuron.neuron_id  # Different seed per neuron
        ) for neuron in pop
    }
    
    # Create emitters with smaller initial sigma for more focused search
    emitters = {
    ...
    for iteration in tqdm(range(config["iterations"])):
        ...
        for neuron in pop:
            ...
            if random.random() < exploration_rate or archive.empty:
                # Exploration: use emitter
                solution = solution
            else:
                # Exploitation: select good solution from archive
                # Use fitness-proportional selection with power scaling
        ...
        n_teams = config.get("n_teams", 30)  # Increase team count
        ...
        for neuron in pop:
            ...
            weight = min(0.8, iteration / (config["iterations"] * 0.5))
